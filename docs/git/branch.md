...
这个命令一般是将某一个提交合并到当前分支，如果需要将多个提交合并到当前分支，推荐使用 `git rebase`。

## `rebase`

首先通过简单的提交节点图解感受一下 `rebase` 在干什么，构造两个分支 `master` 和 `feature`，其中 `feature` 是在提交节点 B 处从 `master` 上拉出的分支。`master` 上有一个新提交 M，`feature` 上有两个新提交 C 和 D。

![](../assets/git/branch_11.png)

此时我们切换到 `feature` 分支上，执行 `rebase` 命令，相当于是想要把 `master` 分支合并到 `feature` 分支。
...
下图为 `rebase` 后的提交节点图，解释一下其工作原理：当执行 `rebase` 操作时，git 会从两个分支的共同祖先开始提取变基分支山的修改，然后将待变基分支(feature)指向基分支(master)的最新提交，最后将刚才提取的修改应用到基分支的最新提交后面。

![](../assets/git/branch_12.png)

结合上面的例子解释：当在 `feature` 分支上执行 `git rebase master` 时，git 会从 `master` 和 `feature` 的共同祖先 B 开始提取 `feature` 分支上的修改，也就是 C 和 D 两个提交，先提取到。然后将 `feature` 分支指向 `master` 分支的最新提交上，也就是 M。最后把提取的 C 和 D 接到 M 后面，实际是依次拿 M 和 C、D 内容分别作比较，处理冲突后生成新的 C' 和 D'。一定注意，这里的 C' 和 D' 和之前的 C、D 已经不一样了，是我们处理冲突后的新内容，`feature` 指针自然最后也指向 D'。

!!! note

    一般解决完冲突以后，还需要将解决后的文件再提交一次，然后执行 `git rebase --continue` 命令完成 `rebase` 操作。之后再切回到 `master` 分支，执行 `git merge feature` 命令，完成代码的更新。

下图是一个 `rebase` 的简单示例，`rebase` 的本质是改变当前分支的祖先，将原先分出时提交节点改变为最新的提交节点。因此，提交记录会比较简洁，但是无法从提交记录中看出当前的分支最早是从哪一个分支拉出来的。

![](../assets/git/branch_8.png)

是否需要 `rebase`，一般根据公司的情况和使用场景来，更多的还是使用 `merge` 来合并分支，可以通过历史提交记录查看每个时间节点干了什么以及先后殊勋。