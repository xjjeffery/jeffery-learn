...
首先将 `.clang-format` 文件保存到一个我们想要存放的路径中，然后打开 settings，搜索 format。

![](./assets/clang_format/vscode_clang_format.png)

在 Clang_format_style 中指定我们保存的 `.clang-format` 路径，这样我们就可以通过自己设置的 clang-format 进行格式化代码。

也可以在 settings 中搜索 format 配置保存自动格式化。

![](./assets/clang_format/vscode_format_on_save.png)
...
![](./assets/clang_format/qt_beautifier.png)
...
![](./assets/clang_format/qt_set_clang_format.png)

然后点击 Use customized style->Add，将自定义的 clang-format 赋值添加进去，最后点击 ok 并退出。

![](./assets/clang_format/custom_format.png)
